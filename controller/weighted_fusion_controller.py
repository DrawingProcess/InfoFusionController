...
# 두 알고리즘의 상태를 결합하는 함수
def combine_states(states_pursuit, states_mpc):
    ...
    for i in range(4):  # [x, y, theta, velocity]
        combined_state[:, i] = 0.5 * states_pursuit[:, i] + 0.5 * states_mpc[:, i]

    return np.array(combined_state)

class WeightedFusionController(BaseController):
    ...
    def follow_trajectory(self, start_pose, ref_trajectory, goal_position, show_process=False):
        ...
        while True:
            ...
            if predicted_states_mpc is None or is_pursuit_reached == False:
                ...
                next_states = combine_states(np.array(predicted_states_pursuit), np.array(predicted_states_mpc))
...
def main():
    ...
    controller = WeightedFusionController(horizon=horizon, dt=dt, wheelbase=wheelbase, map_instance=map_instance)
