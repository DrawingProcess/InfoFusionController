...
def combine_states(states_pursuit, states_mpc, mi):
    ...
    for i in range(4):  # [x, y, theta, velocity]
        ...
        else:  # Mutual Information이 낮을 때
            combined_state[:, i] = states_mpc[:, i]
        # print(f"mi{i}: {mi[i]}, w1: {weight1}, w2: {weight2}")

    return np.array(combined_state)

class InfoFusionController(BaseController):
...
def main():
    ...
    controller = InfoFusionController(horizon=horizon, dt=dt, wheelbase=wheelbase, map_instance=map_instance)
